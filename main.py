...
from configs import CAMERA_INDEX, COLOR_RANGES, DEFAULT_COLOR, SHOW_MASK, WINDOW_NAME, MIN_CONTOUR_AREA, YOLO_WEIGHTS, YOLO_CFG, YOLO_NAMES
from tracking.detector import detect_colored_object, detect_yolo
...
def main():
    ...
    while True:
        ...
        if key in color_keys and mode == "color":
            ...
            mode = "dnn"
            print("[INFO] Switched to DNN mode")
        elif key == ord('c'):
            mode = "color"
            print("[INFO] Switched to color mode")
        ...
        elif mode == 'dnn':  # DNN mode
            results = detect_yolo(frame, net, output_layers, classes)
            for box, label, confidence in results:
                draw_bounding_box(frame, box, label=label, confidence=confidence)
        
        # Display mode and color
        cv2.putText(frame, f"Mode: {mode.capitalize()}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
        if mode == "color":
            cv2.putText(frame, f"Color: {selected_color.capitalize()}", (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
        cv2.putText(frame, "Select: p (pink), b (blue), y (yellow), g (green), w (white), o (orange), d (DNN), c (color)",
